...
local Assets = import("rbxassetid://10790482123").RemoteSpy
...
scriptContext:SetCallback(function()
    ...
    if #selected.args == 0  or not selected.args or type(selected.args)~="number" then
        -- Making sure there's no issues because that should NEVER happen
        ...
        xpcall(function()
            ...
            for i = 1, #selectedArgs do
                ...
                if valueType=="table" then
                    script = script .. ("local ohTable_%d = %s\n"):format(i, v) 
                    args = args .. ("ohTable_%d, "):format(i)
                else
                    args = args .. v
                end
            end
    
            setClipboard(script .. (script~="" and '\n' or "") .. remotePath .. ':' .. method .. '(' .. args:sub(1, -3) .. ')')
        end, function() 
            setClipboard(remotePath .. ':' .. method .. "()")
        end)
    end
